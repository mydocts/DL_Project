...
class CalvinDataset_Goalgen(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # input image (sequence of 20 frames)
        input_images = []
        for i in range(20):
            # Calculate frame index, padding with 0 if needed
            curr_fid = frame_id - 19 + i
            if curr_fid < 0:
                curr_fid = 0
            
            img_path = os.path.join(self.data_dir, f"{traj_id}", f"{curr_fid}_static.png")
        ...
        # Apply transforms
        # Note: We need to apply consistent transforms if using color jitter
        # For simplicity in this modification, we apply transforms individually or stack first
        
        # Stack all images: 20 inputs + 1 goal = 21 images
        all_images = np.stack(input_images + [edited_image], axis=0) # (21, H, W, 3)
        all_images = torch.tensor(all_images).permute(0, 3, 1, 2) # (21, 3, H, W)
        
        # Apply color jitter if needed (omitted for brevity/safety in this step, can add back if requested)
        # Resizing/Cropping
        # We process them as a batch
        all_images = 2 * (all_images / 255.0) - 1.0
        
        # Apply resize/crop transform
        # The self.transform expects (C, H, W) or batch. 
        # Our transform is:
        # transforms.Resize((self.resolution, self.resolution))
        # It works on (..., C, H, W)
        
        all_images = self.transform(all_images)
        
        input_images = all_images[:20] # (20, 3, H, W)
        edited_image = all_images[20]  # (3, H, W)

        example = dict()
        example['input_text'] = [edit_prompt]
        example['original_pixel_values'] = input_images
